...
| `leads.py` | `POST /api/lead` — the email gate: validate, one `leads` INSERT (a duplicate is the same success), one server-side event, the report rendered from the warmed cache and mailed inline (`send_report_for`), the article redirect whatever the mail did | A model call, failing the request over a mail, logging an address, touching a purchase |
...
| `scripts/send_lead_reports.py` | Re-sending, by hand, the lead reports the route could not send (`report_sent_at IS NULL`), through `leads.send_report_for` | A model call, printing an address, being scheduled |
